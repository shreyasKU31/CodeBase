...
// Commit files with specific dates
    ...
    for (let i = 0; i < dates.length && fileIndex < files.length; i++) {
      ...
      for (let j = 0; j < filesPerDate && fileIndex < files.length; j++) {
        ...
        try {
          ...
          const dateEnv = `GIT_AUTHOR_DATE="${date.toISOString()}" GIT_COMMITTER_DATE="${date.toISOString()}"`;
          
          execSync(`${dateEnv} git commit -m "${commitMessage}"`, { 
            stdio: 'pipe',
            shell: true
          });
